...
Clone the git repo:
```bash
git clone https://github.com/TeamAvitab/avitab_georefs
cd avitab_georefs
```
...
```bash
python3 scripts/start_update.py
```

This script performs the following steps

1. determines which AIRAC cycle needs to be updated
2. downloads the previous AIRAC cycle
3. reorganises the previous charts
4. copies matching georeferences from the previous cycle
5. downloads the new AIRAC cycle
6. reorganises the new charts
7. copies matching georeferences to the new cycle
8. starts the interactive similar-chart review tool if any charts still need manual review
9. shows the current georeference status
...
WSL1 (with e.g. XServer setup) is not recommended - primarily due to the GUI for the copy_similar_georefs_gui.py script.
...
Packages selected georeference JSON files into a distributable archive. It takes a variable number of arguments for AIRAC directories, but typically only 2 are used.
...
It can be run with an argument for the 4 digit AIRAC cycle to be updated. It can also be run without an argument in which case it makes a best guess based on the current AIRAC cycle, what AIRAC cycle charts are on NAT AIP and the release available on TeamAvitab.
...
- UK NATS AIP only serves the .zip files for 3 AIRAC cycles at a time - current, next and next + 1. Older .zips are typically not made available. It is essential to update in the 2 x 28 day window while both the previous and new update cycles are available on UK NATS AIP. Ideally, an update would be done for the next AIRAC cycle based on the current AIRAC cycle. Because NATS AIP also provides next + 1, an update could be started in advance for next + 1 based on next. However, charts are subject to change before they get promoted from next + 1 to next. So it is recommended to wait and re-check for the promotion before releasing.
